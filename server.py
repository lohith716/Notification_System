...
def handle_client(conn, addr):
    ...
    while connected:
        ...
        if msg[:7] == "COMS WA":
            conn.send("COMS received".encode(FORMAT))
            recipient_nickname = msg.split()[2]  
            recipient_index = NICKNAME.index(recipient_nickname)
            recipient_conn = CLIENTS[recipient_index][0]
            sender_nickname = NICKNAME[index]
            sender_msg = f"Direct message from {sender_nickname}: {msg}"
            recipient_conn.send(sender_msg.encode(FORMAT))
